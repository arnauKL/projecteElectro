...
void calcularFFT(FFTbuffer *buff) {
  ...
  ArduinoFFT<float> FFT = ArduinoFFT<float>(
      buff->vReal, buff->vImg, samples,
      (float)SAMPLE_FREQ); // li passo les dades reals, imaginaries, el nombre
                           // de samples i la freq. de mostreig

  FFT.windowing(FFTWindow::Hamming,
                FFTDirection::Forward); // finestra i direcció
...
void computeStress(FFTbuffer *buff, float &sns, float &snp, float &stress) {
  ...
  for (int i = 0; i < MAX_BUFFER_RR / 2; i++) {
    ...
    bin =
        i * (SAMPLE_FREQ / MAX_BUFFER_RR); // bin de freq. en el que ens trobem

    if (bin > 0.04 && bin < SN_LIMIT)
      sns += buff->vReal[i]; // LF
    else if (bin > SN_LIMIT && bin < 0.4)
      snp += buff->vReal[i]; // HF
  }
